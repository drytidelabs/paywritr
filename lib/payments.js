...
// Ensure we speak the expected Nostr websocket subprotocol.
class NostrWebSocket extends WS {
  constructor(url, protocols) {
    const p =
      protocols && (Array.isArray(protocols) ? protocols.length : String(protocols).length)
        ? protocols
        : ['nostr'];
    super(url, p, { perMessageDeflate: false });
  }
}
