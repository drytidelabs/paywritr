...
async function nwcCall({ relay, walletPubkey, clientSecretKey, method, params, timeoutMs = 12_000 }) {
...
export async function createInvoice({ provider, amountSats, memo, slug }) {
  ...
  if (provider === 'alby_nwc') {
    const { relay, walletPubkey, secretKey } = parseAlbyNwcUrl(process.env.ALBY_NWC_URL);
