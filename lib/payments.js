...
export function parsePaymentsProvider(env = process.env) {
  ...
  // Unknown provider values should not silently route to LNbits.
  // Default to the canonical provider instead.
  return 'alby_hub';
}
...
function safeUrlForLogs(url) {
  try {
    ...
    // Keep protocol+host only; never log paths/query that could contain secrets.
...
export function classifyProviderError(e) {
  ...
  if (s.includes('closed') || s.includes('econnrefused') || s.includes('econnreset') || s.includes('socket hang up') || s.includes('relay') || s.includes('unavailable')) {
...
export async function createInvoice({ provider, amountSats, memo, slug }) {
  if (provider === 'lnbits') {
    ...
    if (!r.ok) {
      ...
      // Map auth errors to config, 5xx to unavailable, otherwise generic error.
      ...
      const err = new Error(
        isAuth
          ...
          : isServer
            ? `LNbits unavailable (HTTP ${status})`
            : `LNbits error (HTTP ${status})`
      );
      err.statusCode = isAuth ? 500 : 502;
      err.detail = `lnbits=${safeUrlForLogs(LNBITS_URL)} status=${status} body=${redact(text)}`;
      throw err;
    }
